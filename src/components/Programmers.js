...
export default function Programmers() {
  ...
  const [listOfProgrammers] = useState(listOfAwesome);
  ...
  const getNameOfFeatured = () => {
    ...
    return listOfProgrammers[id-1].name
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfProgrammers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {(id === dev.id) ? setId(null) : setId(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
